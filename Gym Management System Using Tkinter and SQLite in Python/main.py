...
def show_login():
    global login_win
    login_win = tk.Tk()
    login_win.title("MASA - Gym Login")
    ...
    tk.Label(login_win, text="Gym Management Login", font=("Segoe UI", 14, "bold"), bg="#ecf0f1").pack(
        pady=10
    )

    tk.Entry(login_win, textvariable=user_var).pack(pady=5)
    tk.Entry(login_win, textvariable=pass_var, show="*").pack(pady=5)
    tk.Button(login_win, text="Login", width=15, command=login).pack(pady=15)
    login_win.mainloop()
...
def main_app():
    root = tk.Tk()
    root.title("MASA - Gym Management System")
    ...
    def get_dashboard_stats():
        ...
        active = cur.execute("SELECT COUNT(*) FROM members WHERE expiry >= ?", (today,)).fetchone()[0]
        expired = total - active
        revenue = cur.execute("SELECT IFNULL(SUM(amount),0) FROM payments").fetchone()[0]
    ...
    def add_member():
        ...
        VALUES (?,?,?,?)
        """,
            (member_id, feesv.get(), datetime.now().strftime("%Y-%m-%d"), pay_mode.get()),
        )
        conn.commit()
    ...
    def update_member():
        ...
        confirm = messagebox.askyesno(
            "Confirm Update", f"Are you sure you want to update member '{namev.get()}'?"
        )
        if confirm:
            cur.execute(
                """
    ...
    def delete_member():
        if not idv.get():
            ...
            return
        confirm = messagebox.askyesno(
            "Confirm Delete", f"Are you sure you want to delete member '{namev.get()}'?"
        )
        if confirm:
    ...
    def check_expiry_alerts():
        ...
        if alert_list:
            messagebox.showwarning(
                "Membership Expiry Alert", "Following members are about to expire:\n" + "\n".join(alert_list)
            )
    ...
    header.pack(fill="x")
    tk.Label(
        header, text="🏋️ Gym Management System", fg="white", bg="#2c3e50", font=("Segoe UI", 20, "bold")
    ...
    def create_card(title, icon, color):
        frame = tk.Frame(cards_frame, bg=color, width=180, height=100)
        frame.pack(side="left", padx=8)
        frame.pack_propagate(False)

        tk.Label(frame, text=title, font=("Segoe UI", 10, "bold"), bg=color, fg="white").pack(
    ...
    left = tk.LabelFrame(
        content, text=" Member Details ", font=("Segoe UI", 11, "bold"), bg="#f4f6f9", padx=15, pady=15
    )
    ...
    ttk.Combobox(left, textvariable=genderv, values=["Male", "Female"], state="readonly", width=23).grid(
        row=2, column=1, pady=5
    )
    tk.Label(left, text="Plan", bg="#f4f6f9").grid(row=3, column=0, sticky="w", pady=5)
    plan_combobox = ttk.Combobox(
        left, textvariable=planv, values=["Monthly", "Quarterly", "Yearly"], width=23
    )
    ...
    expiry_entry = DateEntry(
        left, textvariable=expiryv, width=22, state="readonly", date_pattern="yyyy-mm-dd"
    )
    ...
    ttk.Combobox(left, textvariable=pay_mode, values=["Cash", "Card", "UPI"], width=23).grid(
    ...
    ttk.Button(left, text="Renew Membership", command=renew_membership).grid(
        row=11, column=0, columnspan=2, pady=10
    )

    right = tk.LabelFrame(
        content, text=" Members List ", font=("Segoe UI", 11, "bold"), bg="#f4f6f9", padx=10, pady=10
    )
